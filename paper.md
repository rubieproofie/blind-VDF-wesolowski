...
Let G be a group of unknown order (e.g., class group of an imaginary quadratic field or RSA modulus with secret factorization) with generator g. Fix a public non-square a in G. Let H: G -> primes of bit-length approx lambda be a random oracle.

Wesolowski VDF: On input x in G, T in N, compute y = x^(2^T), ell = H(y), q = floor(2^T / ell), r = 2^T mod ell), pi = x^q. Verify: y = pi^ell * x^r.
...
Blindness: The server's view on a blinded input alpha = x * a^r (r uniform, kappa >> lambda + log T) is computationally/statistically indistinguishable from a random group element, even after seeing the final (y, pi).

---

## 3. Hybrid Construction (Idea 2)

Public parameters: G, a, H, T, S.

1. Client samples r <- [0, 2^kappa), sends alpha = x * a^r.
...
4. Client computes ell = H(y), q = floor(2^T / ell), and uses the unblinded ladder to evaluate pi = x^q via base-2^S windowed exponentiation (O(T/S) time).
5. Output: (y, pi).
...
1. Client blinds and sends alpha = x * a^r.
...
Correctness and security: Algebraic identities hold exactly (see PoC). Blindness and soundness reduce to standard Wesolowski assumptions; the interactive challenge resolves Fiat-Shamir circularity without new primitives.
...
Open question: Can the native construction be made non-interactive while preserving succinctness and O(polylog T) client work?
